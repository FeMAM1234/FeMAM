...
class client():
    def __init__(self,i,args,data,model):
        ...
        self.stop_layer=False
        self.eval_acc=[999]
        self.eval_loss=[999]
        self.train_acc=[999]
        self.process_mask=[0,True]
    def load_model(self,global_structure):
        layers = [list(global_structure.keys())[-1]] if self.leaf_only else list(global_structure.keys())
        # layers = [len(global_structure)-1] if self.leaf_only else list(range(len(global_structure)))
        update=False
        for layer in layers:
            for group_id,group in global_structure[layer].items():
                if self.id in group[0]:
                    ...
                    update=True
        if self.leaf_only and update==False:
            self.stop_layer=True
    def train(self):
        train_loader = generate_dataloader(self.args.data_name, self.data['train'], batch_size=32)
